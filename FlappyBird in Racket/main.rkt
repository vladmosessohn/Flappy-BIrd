...
(define-struct flappy-bird (x y v-y) #:transparent)
(define-struct pipe (gap-y x) #:transparent)
(define-struct mult-pipes (pipes) #:transparent)
(define-struct actualstate (flappy-bird mult-pipes score) #:transparent)
...
(define initial-bird (flappy-bird bird-x bird-initial-y 0))

(define (get-initial-state)
  (let ([y (+ added-number (random random-threshold))]) (actualstate initial-bird (list (pipe y scene-width)) 0)))
...
(define (next-state-bird bird gravity)
  (struct-copy flappy-bird bird [y (+ (get-bird-y bird) (flappy-bird-v-y bird))] [v-y (+ (flappy-bird-v-y bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy flappy-bird bird [v-y (- momentum)]))
...
(define (pressed-space-state current-state)
  (struct-copy actualstate current-state [flappy-bird (next-state-bird-onspace (get-bird current-state) initial-momentum)]))

(define (change current-state pressed-key)
  (if (equal? pressed-key " ") (pressed-space-state current-state) current-state))
...
(define (move-pipes pipes scroll-speed)
  (if (null? pipes)
      '()
      (cons (struct-copy pipe (car pipes) [x (- (get-pipe-x (car pipes)) scroll-speed)])
...
(define (clean-pipes pipes)
  (filter (lambda (z) (> (+ (get-pipe-x z) pipe-width) 0)) pipes))
...
; cu pasărea.
(define (count-pipes pipes)
  (if (null? pipes)
      0
      (+ 1 (count-pipes (cdr pipes)))))

(define (last-pipe-get-x pipes)
  (get-pipe-x (last pipes)))

(define (add-more-pipes pipes)
  (let*
      ([y (+ added-number (random random-threshold))] [x (+ (+ (last-pipe-get-x pipes) pipe-gap) pipe-width)])
    (if (< (count-pipes pipes) no-pipes)
      (append pipes (list (pipe y x)))
      pipes)))
...
(define (check-two-rect bird pipes)
  (if (or (check-collision-rectangles (make-posn (get-pipe-x (car pipes))
                                                     (+ (get-pipe-gap-y (car pipes)) pipe-self-gap))
                                          (make-posn (+ (get-pipe-x (car pipes)) pipe-width)
                                                     ground-y)
                                          (make-posn bird-x
                                                     (get-bird-y bird))
                                          (make-posn (+ bird-x bird-width)
                                                     (+ (get-bird-y bird) bird-height))
                                         
                                          )
          (check-collision-rectangles (make-posn (get-pipe-x (car pipes))
                                                     0)
                                          (make-posn (+ (get-pipe-x (car pipes)) pipe-width)
...
(define (next-state state)
  (let ([new-score (+ (get-score state) 0.1)])
  (actualstate (next-state-bird (get-bird state) initial-gravity) (next-state-pipes (get-pipes state) initial-scroll-speed) new-score)))
...
(define (place-pipes pipes scene)
  ...
  (if (null? pipes)
      ...
      (let* ([carp (car pipes)]
             [cdrp (cdr pipes)]
             [x (+ (get-pipe-x carp) (/ pipe-width 2))]
             [y1 (quotient (get-pipe-gap-y carp) 2)]
             [y2 (+ (+ (get-pipe-gap-y carp) pipe-self-gap) (quotient (- (- scene-height pipe-self-gap) (get-pipe-gap-y carp)) 2))]
             [list-pos (list (make-posn x y1) (make-posn x y2))]
             [rect1 (rectangle pipe-width (get-pipe-gap-y (car pipes)) "solid" "green")]
             [rect2 (rectangle pipe-width ( - ( - scene-height (get-pipe-gap-y (car pipes)))pipe-self-gap)  "solid" "green")]
             [list-rect (list rect1 rect2)])
        (place-pipes cdrp (place-images list-rect
                                        list-pos
                                        scene)))))
; Bonus
